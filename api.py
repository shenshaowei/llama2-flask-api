...
parser.add_argument('--tokenizer_path', type=str, default='/home/a/Documents/llama2/Llama2-7b/7B/tokenizer.model', help='Path to tokenizer model (str)')
...
def run(rank, size, request_queue, response_queue):
    ...
    while True:
        ...
        print(dialogs)
        dialogs=[i[:256] for i in dialogs]
        print(dialogs)
        ...
        for result in results:
            comps.append(result['generation'])
        

        response_queue.put(comps)
...
def message_route():
    ...
    '''if request.json.get("stream"):
        ...
        for delta in deltas:
            ...
            output =respond_json(delta_response, "delta")) 
        return output'''
